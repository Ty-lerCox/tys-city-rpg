...
// ============================================================
datablock fxDTSBrickData(ATMWData)
{
    brickFile = "./shapes/Bricks/ATMW.blb";

	category = "CityRPG";
	subCategory = "CityRPG Infoblock";
	
	uiName = "Cuprum's ATM - Wall";
...
function CityRPGplayerATMBrickData::parseData(%this, %brick, %client, %triggerStatus, %text)
{
	if(%triggerStatus !$= "")
	{
		...
		{
			messageClient(%client, '', "\c6Welcome to the Copper Bank ATM.");
			if(CityRPGData.getData(%client.bl_id).valueBank > 0)
			{
				messageClient(%client, '', "\c6- You have \c3$" @ CityRPGData.getData(%client.bl_id).valueBank SPC "\c6in your account.");
			}
