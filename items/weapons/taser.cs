...
{
	AddDamageType("Taser",   '<bitmap:Add-Ons/GameMode_TysCityRPG/shapes/ci/taser> %1',    '%2 <bitmap:Add-Ons/GameMode_TysCityRPG/shapes/ci/taser> %1', 0.5, 1);
	...
	datablock ItemData(taserItem)
	{
		...
		shapeFile = "Add-Ons/GameMode_TysCityRPG/shapes/taser.dts";
		rotate = false;
		mass = 1;
		density = 0.2;
		elasticity = 0.2;
		...
		uiName = "Taser";
		iconName = "Add-Ons/GameMode_TysCityRPG/shapes/ItemIcons/taser";
		doColorShift = false;
...
function taserProjectile::onCollision(%this, %obj, %col, %fade, %pos, %normal)
{
	if((%col.getType() & $typeMasks::playerObjectType) && isObject(%col.client))
	{
		...
		if(CityRPG_illegalAttackTest(%obj.client, %col.client))
		{
...
function freezeLoop(%this)
{
	...
	if(%this.loops >= 20)
	{
		%this.setDataBlock(playerCityArmor);
		%this.loops = "";
		return;
	}
