...
datablock ItemData(knifeItem)
{
    ...
    shapeFile = "Add-Ons/Gamemode_TysCityRPG/shapes/knife2.dts";
    mass = 1;
    density = 0.2;
    elasticity = 0.2;
    friction = 0.6;
    ...
    //gui stuff
    uiName = "Knife";
    iconName = "Add-Ons/Gamemode_TysCityRPG/shapes/knifeicon";
    doColorShift = true;
...
/////////////////////////::onCollision(%this,%obj,%col,%fade,%pos,%normal,%brick)
function knifeProjectile::onCollision(%this,%obj,%col,%fade,%pos,%normal,%brick)
{
	if(%col.getClassName() $= "fxDTSBrick")
    {
        ...
        if(%brickData.isDrug)
        {
			...
			{
				if(%client.lotCooldown > 2)
					messageClient(%client,'',"Please slow down");
				else {
					%target = %client;
					%newBrickID = %col;
					...
					%client.lotCooldown++;
					messageClient(%client, '', "\c3Processed Transfer.");
				}
			} else
				messageClient(%client,'',"Not your lot to transfer.");
		}
	}
