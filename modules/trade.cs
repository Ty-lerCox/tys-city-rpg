        ...
        if(!(%arg2 $= "") && !(%arg3 $= ""))
        {
            ...
            if(isObject(%target))
			{
                if(getWord(CityRPGData.getData(%client.bl_id).valueResources, 0) >= %arg2)
                {
                    ...
                    messageClient(%client, '', "\c6You have began a trade with\c3" SPC %target.name SPC "\c6. Selling him/her\c3" SPC %arg2 SPC "Lumber \c6for \c3$" @ %arg3 @"\c6.");
				    messageClient(%target, '', "\c3" @ %client.name SPC "\c6has began a trade with you. Selling you\c3" SPC %arg2 SPC "Lumber \c6for \c3$" @ %arg3 @ "\c6.");
