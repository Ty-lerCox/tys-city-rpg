...
function JobSO::populateJobs(%so)
{
	...
	%so.addJobFromFile("policeman");              // 3 
	%so.addJobFromFile("policelieutenant");       // 4 
	%so.addJobFromFile("criminal");               // 5
	%so.addJobFromFile("bountyhunter");           // 6
	%so.addJobFromFile("policechief");            // 7
	%so.addJobFromFile("miner");                  // 8
	%so.addJobFromFile("shopkeeper");             // 9
	%so.addJobFromFile("armsdealer");             // 10
	%so.addJobFromFile("lumberjack");             // 11
	%so.addJobFromFile("laborer");                // 12
	%so.addJobFromFile("tailor");                 // 13
	%so.addJobFromFile("councilmember");          // 14
	%so.addJobFromFile("administrator");          // 15
	%so.addJobFromFile("mobster");                // 16
	%so.addJobFromFile("mobboss");                // 17
	%so.addJobFromFile("godfather");              // 18
	%so.addJobFromFile("mayor");                  // 19
	%so.addJobFromFile("shopowner");              // 20
	%so.addJobFromFile("shopceo");                // 21
	%so.addJobFromFile("professionallaborer");    // 22
	%so.addJobFromFile("medicalassistant");       // 23
	%so.addJobFromFile("medicalinturn");          // 24
	%so.addJobFromFile("doctor");                 // 25
	%so.addJobFromFile("surgeon");                // 26
	%so.addJobFromFile("president");		  		// 27
	%so.addJobFromFile("private");		    		// 28
	%so.addJobFromFile("mastersergeant");		    // 29
	%so.addJobFromFile("armylieutenant");		    // 30
	%so.addJobFromFile("vicepresident");		    // 30
	%so.addJobFromFile("rapist");		    		// 31
}
...
function CalendarSO::loadCalendar(%so)
{
	...
	%so.holiday[100] = "\c6It's <color:FF9900>Bay's \c6birthday today!";
	%so.holiday[95] = "\c6It's <color:FF9900>Rairai's \c6birthday today!";
	%so.holiday[91] = "\c2A\c1p\c2r\c1i\c2l \c0F\c3o\c0o\c3l\c0s \c7D\c6a\c7y\c6!";
	%so.holiday[110] = "\c6It's \c24/20 \c6today.";
	%so.holiday[156] = "\c2It's Rick Roll day! \c7(Grr, I hate that guy)";
	%so.holiday[265] = "\c1It's \c5Random\c3 Day\c4!\c2!";
	%so.holiday[327] = "\c6It's <color:FF9900>?-Despair-?'s \c6birthday today!";
